"""Evidence retriever for open-world product representations."""
...
def retrieve(query: str, max_items: int = 5) -> List[EvidenceProduct]:
    """Retrieve evidence products using the research agent."""
    ...
    for idx, insight in enumerate(insights[:max_items]):
        ...
        evidence.append(
            EvidenceProduct(
                id=insight.get("id") or f"evidence-{idx + 1}",
                name=title,
                description=summary,
                source=insight.get("source") or "research",
