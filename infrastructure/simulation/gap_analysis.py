"""Infrastructure gap analysis wrapper (embeddings + env thresholds).

Pure gap logic lives in `domain.simulation.gap_analysis`.
"""
...
def analyze_gap(
    ...
    if winner and winner.id != product.id:
        ...
        winner_matched, _ = _matched_and_missing(
            goal, tokens, winner_dict, winner_tokens
        )
