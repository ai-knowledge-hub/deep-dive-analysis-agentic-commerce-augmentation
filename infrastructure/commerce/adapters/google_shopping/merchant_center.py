...
from domain.commerce.types import Product, RawOffer
from shared.transformers.offers import convert_offers
from shared.transformers.normalize import transform_catalog
...
def _entry_to_offer(entry: dict) -> RawOffer:
    ...
    return RawOffer(
        ...
        completeness=0.7,
        inferred_fields=[
            key for key in ["capabilities", "intent_scores"] if key not in entry
        ],
    )
