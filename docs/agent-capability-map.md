...
| External agents may call the platform | External-agent principal plus job/run API | `POST /external-agent/jobs`, `GET /external-agent/jobs/{job_id}`, `GET /external-agent/jobs/{job_id}/receipt`, `GET /external-agent/jobs/{job_id}/receipts`, `GET /external-agent/jobs/{job_id}/activity`, `GET /external-agent/jobs/{job_id}/events`, `GET /external-agent/credentials/metadata`, `GET /agent-runs/registry`, scoped principal resolution, registry-pinned skills/tools | Idempotent job facade, signed receipt history, scoped event feed, normalized activity projection, scoped credential metadata, and protocol discovery/readiness domain summaries implemented |
...
| External-agent job contracts | Idempotent job create/status facade exists, links jobs to agent runs, signs latest-status receipts, stores receipt history, exposes scoped linked-run events, projects normalized activity, summarizes domain-specific evidence, and publishes credential scope metadata for least-privilege tool/skill access. | Add issuer/admin APIs for credential issuance, rotation, and revocation as well as retry-safe contracts for any remaining endpoint gaps. |
| Harness profiles | `harness_id` is behavior-defining for run creation: defaults resolve from agent profile, run-mode/policy mismatches are rejected, recovery uses harness fallback order, `allowed_effect_classes` blocks out-of-bound planned capabilities, `planner_mode` shapes the initial action queue, stopping conditions pause/complete runs at runtime, and `memory_policy=no_mutation` blocks registry-declared learning/memory mutation capabilities. | Continue expanding memory policy coverage as more learning or memory mutation capabilities are added. |
