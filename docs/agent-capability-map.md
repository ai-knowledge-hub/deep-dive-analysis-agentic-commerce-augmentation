...
| Harness profiles | `harness_id` is behavior-defining for run creation: defaults resolve from agent profile, run-mode/policy mismatches are rejected, recovery uses harness fallback order, and `allowed_effect_classes` blocks out-of-bound planned capabilities such as write actions under `observe_only`. | Continue making planner mode, memory policy, and stopping conditions affect runtime behavior beyond registry visibility. |
