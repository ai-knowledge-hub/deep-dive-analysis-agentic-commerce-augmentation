...
- Persistence:
  ...
  - Tool owner/steward metadata can be updated through a scoped registry ownership endpoint; ownership updates require dry-run preflight plus explicit confirmation and then create a new registry release fingerprint through the existing transition audit path.
...
- Control-plane UI:
  ...
  - Agent Runs selected-action detail shows registry owner, steward, and ownership source metadata, with a gated ownership edit form for the selected tool.
