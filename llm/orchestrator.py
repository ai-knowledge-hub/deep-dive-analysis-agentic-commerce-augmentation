...
def render_context(packet: ContextPacket, include_turns: int = 6) -> str:
    ...
    semantic = ", ".join(packet.semantic_goals) if packet.semantic_goals else "None recorded"
    latest_episode = packet.latest_episode or {}
    episode_text = latest_episode.get("takeaways") or latest_episode.get("outcome") or "No reflections yet"
...
def context_for(
    ...
    packet = build_context(manager, include_turns=include_turns, extra_metadata=extra_metadata)
...
def values_context(
    ...
    packet = build_context(manager, include_turns=include_turns, extra_metadata=state_metadata)
    return packet, render_context(packet, include_turns=include_turns)
