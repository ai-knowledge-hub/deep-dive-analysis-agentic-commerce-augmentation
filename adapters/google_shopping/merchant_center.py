...
from core.transformers.normalize import transform_catalog
from core.transformers.offers import convert_offers
...
def _validate_entry(entry: dict) -> None:
    ...
    if missing:
        raise ValueError(f"Missing required Merchant Center fields: {', '.join(sorted(missing))}")
...
def _entry_to_offer(entry: dict) -> RawOffer:
    ...
    return RawOffer(
        ...
        confidence=0.8,
        completeness=0.7,
        inferred_fields=[key for key in ["capabilities", "empowerment_scores"] if key not in entry],
    )
...
def load_offers(path: str | None = None) -> List[RawOffer]:
    ...
    if not feed_path:
        raise RuntimeError("GOOGLE_MERCHANT_FEED_PATH must be set for google_merchant source")
